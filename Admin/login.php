...
if(isset($_POST['login']))
{
	...
	$sql = "SELECT * FROM adminuser WHERE EMAIL = '{$loginid}' && PASSWORD = '{$password}'";
	$result = mysqli_query($conn, $sql);
	if(mysqli_num_rows($result)==0)
	{
		$sql = "SELECT * FROM adminuser WHERE CONTACT = '{$loginid}' && PASSWORD = '{$password}'";
	    $result = mysqli_query($conn, $sql);
		if(mysqli_num_rows($result)==0)
		{
			echo "<script>alert('Id or Password is Incorrect');
		...
		<div class="container" style="margin-top: 85px;">
		  ...
		  <hr class="bg-yellow" style="height: 2px;">
		   <form class="shadow" action="<?php $_SERVER['PHP_SELF'];?>" method="post">
			 ...
			 <div align="center">
			   <input type="submit" class="btn bg-yellow" value="Login" name="login"/> <span style="padding-left: 13px;"><a href="forgotpassword.php">Forgot Password?</a></span>
			 </div>
		   </form>
		</div> <!-- container div -->
		<br>
